...
export default function CloudFishLoginPage() {
  ...
  useEffect(() => {
    ...
    if (token && !next) {
      window.location.href = '/cloudfish/purchase';
      return;
    }
    setChecking(false);
  }, []);
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 py-16 bg-slate-50 pt-24">
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg border border-slate-200 p-8">
        ...
        <p className="text-slate-600 text-sm mb-6">Use the same credentials as the CloudFish app.</p>
        {error && (
        ...
        <p className="mt-6 text-center text-sm text-slate-600">
          ...
          <Link href="/cloudfish/signup" className="text-teal font-semibold hover:underline">
            Sign up
          </Link>
        </p>
      </div>
